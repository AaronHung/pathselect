...
⚠️ **兩個數字的口徑（DR-045）**：**88.6%** = 全部 arm、學完 T4 後（3708/4185）；
**84.5%** = 只算 A5、學完 T4 後（1179/1395，即 `HIER.md` 結構性診斷表那一列）。
兩者都對，差在範圍。逐 arm：A3 89.5%、A5 84.5%、A5nG 91.8%。
...
### C-11 記憶體效率：同容量為主、4× 為輔（DR-042 修訂 A）

**主要主張（同容量，不需跨容量比較）**：在所有測試的記憶體預算（64–1024）下，
A5 在 task-IL 上皆優於 replay-only；**5 個容量中 4 個 systematic**，含最大的
1024（+2.49 ± 2.42，5/5）。→ A3 拿到 1024（已超出 CONTRACT-3 的 512）仍然輸，
**這個結論不依賴任何跨容量比較**。

**輔助主張（跨容量）**：A5@128 − A3@512 = **+3.06 ± 2.80（5/5）** → 測試範圍內
**4×** 記憶體效率。引用時**三個限定必帶**：

- (a) 錨點為 A3@512；對 A3@1024（即 8×）只有 4/5，**8× 不成立**
- (b) **A3 曲線在測試範圍內未飽和**（class-IL 到 1024 仍在上升）→ 4× 是**下界**，
  不是 A3 需求的上界
- (c) **限於 task-IL**；class-IL 在 |M|=1024 落入雜訊（+1.88 ± 5.69，2/5）

⚠️ **「2×」降為 flat 限適用**（DR-019 已 SUPERSEDED-BY DR-042）。flat 版的防禦
「A3 在 256 後不再改善」**在階層版不成立**（256→0.7596、1024→0.7858 仍在改善），
不得沿用。**「8×」已撤回** —— 原錨點 128 的 class-IL 只有 4/5 且 std(7.71) > mean(7.72)。

### C-12 n < 5 的批次

**2026-08-26 更新**：目前仍是 3-seed 的批次只剩兩類 ——
**E3（β_u 消融）** 與 **main order 的 A1 / A2 / A4 / R1 / R2**。
其 3/3 只能讀作「方向一致」，寫進論文前須回到 5-seed 確認（憲法 §1.2）。

已補到 5 seeds、**不再受此條限制**的：元件消融的 **B1 / B2**（DR-033）、
main order 的 **A3 / A5**（`outputs/exp2/order_main/EXP2.md` 的「n seeds」欄逐臂標示）。
