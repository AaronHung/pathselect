...
### 10.5 E3 — 選片 vs 加權拆解（A5 full flat、fold 1、reverse、seeds 0–4；只評估）

`run_exp2.py` 不存權重，故以 `scripts/run_dr051_e3.py` 包裝 `run_exp2.evaluate`
重跑 A5 並存各 stage 的 F_g/F_p（訓練路徑零改）。三道自檢全過：**重跑的 per_slide 與
`main/` 既有紀錄 5 seed × 569 筆共同欄位逐筆相同**（決定性）；最終 ckpt 經 `run_rounds`
重現 stage-3 選片與權重 184/184 張 × 5 seed；(a)(b)(d) 與紀錄值相符。

四格 CE（舊任務 test 切片；P_own = 學完該任務時選的 8 片，P_final = 學完四個任務後選的；
s_new = 最終 selector 分數）：(a) P_own·等權 (b) P_final·等權 (c) P_own·s_new (d) P_final·s_new（實際）。
...
逐切片 ΔCE（正 = 退步；三舊任務等權平均，5 seed mean ± sd）：

| 分解 | ΔCE | 逐 seed |
|---|---|---|
| (a)→(b) **選片改變**（等權） | **+0.7429 ± 0.3094** | +0.740, +0.937, +0.551, +1.136, +0.351 |
| (b)→(d) **重加權**（P_final） | **−0.0025 ± 0.0119** | +0.015, −0.013, −0.007, −0.011, +0.003 |
| (a)→(c) 重加權（P_own） | −0.0119 ± 0.0203 | |
| (c)→(d) 選片改變（s_new） | +0.7524 ± 0.2984 | |
| (a)→(d) 合計 | +0.7404 ± 0.3038 | |

逐任務：esca +1.942（選片）／−0.008（重加權）；rcc +0.314／−0.001；brca −0.028／+0.001。
退步切片比例（選片改變）：esca 0.787、rcc 0.455、brca 0.594。

讀法（描述性）：舊任務的 CE 上升幾乎全部來自**換了選哪 8 片**；同一組片上 softmax(s_new)
相對等權的重加權貢獻 ≈ 0（|ΔCE| < 0.02，且五 seed 正負不一）。恆等式自檢：(a)→(b) 的
三任務平均 +0.7429 = −ΔU(M1)（§10.3 的 A5 M1 = −0.7429），由 `utility_total` 的
telescoping 定義由構造成立 —— 即 **Table 3 的 ΔU（逐切片口徑）就是「選片改變」那一項**。
...
### 11.4 B4 — 固定頭・正向十折 pod 對照（2026-09-15 完成，78 分鐘）＋同平台配對

| 臂（固定頭，pod，forward） | ACC | Masked ACC | Forgetting | BWT | n |
|---|---|---|---|---|---|
| A5 flat | 0.849 ± 0.030 | 0.917 ± 0.019 | 0.040 ± 0.030 | −0.033 ± 0.034 | 10 |
| A5 hier | 0.823 ± 0.041 | 0.905 ± 0.030 | 0.065 ± 0.035 | −0.055 ± 0.036 | 10 |

hier − flat（固定頭 pod，forward）：ACC −0.0267（hier 較佳 4/10）、Masked −0.0117（4/10）、Forgetting +0.0249（3/10）
—— 正向下 hier 落後 flat 的方向與 Mac（−0.0295，1/10）一致，pod 上折數較分散。

**累積 − 固定（同平台 pod，逐折，forward）**：

| 架構 | ACC | Masked ACC | Forgetting | BWT |
|---|---|---|---|---|
| hier | +0.0074（累積較高 6/10） | +0.0070（7/10） | +0.0044（累積較低 4/10） | −0.0079（4/10） |
| flat | −0.0146（4/10） | −0.0114（4/10） | **+0.0366**（1/10） | −0.0341（2/10） |

只報告：正向下累積式頭與固定頭在 ACC／Masked 上差 < 1.5 pp、折數 4–7/10；Forgetting 在 flat 高 3.7 pp
（9/10 折固定頭較低），hier 兩者相近。與反向（§11.2：ACC 低 4–5 pp）相比，正向的口徑差小得多 ——
正向從 4 類任務起步、C_t 的峰值效應較弱。

平台差（固定頭 pod − Mac，forward）：hier ACC +0.0116（7/10）、flat +0.0088（7/10）；Forgetting hier +0.0038、flat −0.0148。
來源：`outputs/exp3/B4.md`、`sota_fixed/per_slide/`（40 檔）。

### 11.5 B5 — 累積式頭・fold-1 五 seed 六臂（flat、reverse；pod；2026-09-15 完成，115 分鐘）

| 臂（累積式頭） | class-IL | task-IL | 洩漏率 | Jaccard | ΔU(M1, C_t) | ΔU(M1, all8) |
|---|---|---|---|---|---|---|
| A2 LoRA merge only | 44.45 ± 11.36 | 81.41 ± 3.39 | 47.95 ± 13.00 | 0.0021 | −3.971 | −4.360 |
| A3 + Replay | 76.99 ± 1.40 | 90.17 ± 1.85 | 15.04 ± 1.31 | 0.0556 | −0.271 | −0.640 |
| A4 + Replay + KD | 75.62 ± 3.31 | 90.56 ± 1.82 | 16.19 ± 2.75 | 0.1645 | −0.389 | −0.762 |
| A5 full | 75.90 ± 4.37 | 91.31 ± 1.28 | 16.33 ± 4.15 | 0.0894 | −0.367 | −0.738 |
| B1 只 KD | 57.34 ± 12.37 | 84.55 ± 5.25 | 34.82 ± 13.27 | 0.0494 | −2.615 | −2.995 |
| B2 只 eq | 64.42 ± 2.60 | 89.15 ± 3.15 | 27.51 ± 0.51 | 0.0361 | −2.266 | −2.647 |

配對（累積式頭；逐 seed；勝 = A 較佳）：

| 配對 | class-IL | task-IL | 洩漏率 | Jaccard | ΔU(all8) |
|---|---|---|---|---|---|
| A5 − A3 | −1.10 ± 4.78（1/5） | +1.15 ± 2.48（4/5） | +1.29（1/5） | +0.0338（4/5） | −0.0987（3/5） |
| A5 − A4 | +0.28 ± 3.57（4/5） | +0.75（3/5） | +0.14（2/5） | −0.0751（0/5） | +0.0231（3/5） |
| A4 − A3 | −1.38 ± 3.48（2/5） | +0.40（3/5） | +1.15（2/5） | +0.1088（5/5） | −0.1218（2/5） |
| A5 − B1 | +18.55（5/5） | +6.76（5/5） | −18.49（5/5） | +0.0400（4/5） | +2.256（5/5） |
| A5 − B2 | +11.48（5/5） | +2.17（4/5） | −11.18（5/5） | +0.0533（4/5） | +1.909（5/5） |

只報告：累積式頭下，replay（A3）以上的三臂 class-IL 在 75.6–77.0 之間、A5 − A3 為 −1.10（1/5）——
Mac 固定頭的 A5 − A3 = +4.61（5/5）在此設定下沒有出現；A5 相對只 KD／只 eq 的優勢仍為 5/5。
是平台差還是口徑差，待 B6（固定頭 pod 六臂）在 §11.6 配對。ΔU 的 C_t 口徑（早期 stage 類別少、U 較大）
與 all8 口徑相差 0.3–0.4，次序不變。來源：`outputs/exp3/B5.md`、`ablation_acc/per_slide/`（30 檔）。

### 11.6 B6 — 固定頭・fold-1 五 seed 六臂 pod 對照（2026-09-15 完成，110 分鐘）＋同平台配對

| 臂（固定頭，pod） | class-IL | task-IL | 洩漏率 | Jaccard | ΔU(M1) |
|---|---|---|---|---|---|
| A2 | 43.98 ± 5.43 | 80.49 ± 2.61 | 48.68 ± 5.18 | 0.0005 | −5.059 |
| A3 | 78.60 ± 1.59 | 90.06 ± 1.73 | 14.03 ± 1.54 | 0.0598 | −0.720 |
| A4 | 82.42 ± 2.83 | 91.58 ± 0.93 | 10.69 ± 1.48 | 0.1526 | −0.571 |
| A5 | 81.22 ± 1.00 | 89.10 ± 1.15 | 9.81 ± 0.80 | 0.1335 | −0.555 |
| B1 | 56.78 ± 6.87 | 86.42 ± 1.43 | 35.39 ± 7.96 | 0.0471 | −3.223 |
| B2 | 78.21 ± 3.20 | 90.08 ± 1.25 | 13.92 ± 3.61 | 0.0838 | −1.079 |

配對（固定頭 pod）：A5 − A3 class-IL **+2.62（5/5）**、task-IL −0.96（1/5）、洩漏率 −4.22（5/5）、Jaccard +0.0737（5/5）；
A4 − A3 +3.81（4/5）；A5 − A4 −1.20（1/5）；A5 − B1 +24.44（5/5）；A5 − B2 +3.01（3/5）。
固定頭下 A5 > A3 的 class-IL／洩漏率／Jaccard 型態在 pod 上與 Mac（+4.61，5/5）方向一致，量級較小。

**累積 − 固定（同平台 pod，逐 seed，只報告）**：

| 臂 | class-IL（pp） | task-IL（pp） | 洩漏率（pp） | Jaccard | ΔU(M1, all8) |
|---|---|---|---|---|---|
| A2 | +0.47（3/5） | +0.92（3/5） | −0.74 | +0.0016 | +0.699 |
| A3 | −1.61（0/5） | +0.11（3/5） | +1.01 | −0.0042 | +0.080 |
| A4 | **−6.80（0/5）** | −1.01（2/5） | +5.49（累積較高 5/5） | +0.0119 | −0.191 |
| A5 | **−5.32（1/5）** | +2.21（5/5） | +6.52（4/5） | −0.0441 | −0.183 |
| B1 | +0.56（3/5） | −1.87（2/5） | −0.56 | +0.0023 | +0.229 |
| B2 | **−13.79（0/5）** | −0.93（3/5） | +13.59（5/5） | −0.0477 | −1.569 |

只報告：口徑差集中在**含蒸餾或 hinge 的臂**（A4 −6.8、A5 −5.3、B2 −13.8 pp class-IL；洩漏率同幅上升），
無保存（A2）與只 replay（A3）幾乎不動（|Δ| ≤ 1.6 pp）。因此 §11.5「累積式頭下 A5 − A3 = −1.10（1/5）」
是**口徑**造成、不是平台：同平台固定頭仍是 +2.62（5/5）。B2（只 eq）在累積式頭下的 −13.8 pp 指向
hinge 以 C_old 口徑計算時的行為改變，需另案（DR）追查。
平台差（固定頭 pod − Mac）：六臂 class-IL 在 −3.2…+2.7 pp 之間、折數 1–3/5，皆在 seed 間變異內。
來源：`outputs/exp3/B6.md`、`ablation_fixed/per_slide/`（30 檔）。

### 11.7 B7 — Table 2 鏈前兩列（A1 無保存、A3 只 replay）在累積式頭下，flat 十折雙序（pod；2026-09-15；B7-1 48 分鐘、B7-2 55 分鐘）

| 臂（累積式頭，flat） | 順序 | ACC | Masked ACC | Forgetting | BWT |
|---|---|---|---|---|---|
| A1 sequential FT（無 LoRA、無保存） | reverse | 0.481 ± 0.057 | 0.808 ± 0.038 | 0.584 ± 0.077 | −0.584 ± 0.077 |
| A3 LoRA + replay | reverse | 0.819 ± 0.042 | 0.915 ± 0.024 | 0.121 ± 0.050 | −0.111 ± 0.050 |
| A5 full（B1） | reverse | 0.803 ± 0.038 | 0.922 ± 0.018 | 0.134 ± 0.045 | −0.123 ± 0.054 |
| A1 | forward | 0.549 ± 0.099 | 0.783 ± 0.053 | 0.478 ± 0.125 | −0.478 ± 0.125 |
| A3 | forward | 0.831 ± 0.044 | 0.909 ± 0.036 | 0.078 ± 0.044 | −0.067 ± 0.049 |
| A5 full（B3） | forward | 0.835 ± 0.031 | 0.906 ± 0.024 | 0.077 ± 0.036 | −0.067 ± 0.036 |

逐折配對（累積式頭，flat）：

| 配對 | 順序 | ACC | Masked ACC | Forgetting |
|---|---|---|---|---|
| A3 − A1 | reverse | **+0.3380（10/10）** | +0.1069（10/10） | **−0.4629（10/10）** |
| A5 − A3 | reverse | −0.0158（2/10） | +0.0067（7/10） | +0.0124（A5 較低 4/10） |
| A3 − A1 | forward | **+0.2825（10/10）** | +0.1260（10/10） | **−0.4003（10/10）** |
| A5 − A3 | forward | +0.0037（5/10） | −0.0038（3/10） | −0.0012（5/10） |

讀法：累積式頭下，replay 相對無保存的增益（+28–34 pp ACC、10/10）與固定頭同形；
蒸餾＋hinge 相對只 replay **沒有可辨識的 ACC 增益**（reverse −1.6 pp、2/10；forward +0.4 pp、5/10），
與 §11.5／11.6（fold-1 五 seed）及 DR-053 的機制一致：累積式頭下 hinge 以 C_old 量效用，
不約束對後加類別的洩漏。Mac 固定頭參考（同設定、不同平台）：A1 reverse 0.452／forward 0.529，
A3 reverse 0.821／forward 0.841；累積式 − Mac 固定的差 A1 +0.028／+0.020、A3 −0.003／−0.010，
A1／A3 對口徑幾乎不敏感（它們沒有 hinge）。
來源：`outputs/exp3/B7.md`、`sota_acc/per_slide/`（80 檔）。

### 11.8 DR-054 — 合規 hinge：U_old 以當前 C_t 由 P_old 重算（`--uold current`；累積式頭、flat、pod）

第一階段（fold 1、seeds 0–4）：

| 臂 | 設定 | class-IL | task-IL | 洩漏率 | Jaccard |
|---|---|---|---|---|---|
| A5 | 累積式 + U_old 當前 C_t（DR-054） | **79.51 ± 1.70** | 90.94 ± 1.45 | 12.73 ± 2.12 | 0.1250 |
| A5 | 累積式 + U_old 快照 C_old（B5） | 75.90 ± 4.37 | 91.31 | 16.33 | 0.0894 |
| A5 | 固定頭 pod（B6） | 81.22 ± 1.00 | 89.10 | 9.81 | 0.1335 |
| B2 | 累積式 + U_old 當前 C_t（DR-054） | **78.87 ± 2.79** | 89.29 | 13.05 | 0.0703 |
| B2 | 累積式 + U_old 快照 C_old（B5） | 64.42 ± 2.60 | 89.15 | 27.51 | 0.0361 |
| B2 | 固定頭 pod（B6） | 78.21 ± 3.20 | 90.08 | 13.92 | 0.0838 |

凍結判準 A5_ucur − A3（B5 的 A3，76.99）：**+2.52 ± 1.90 pp，5/5** → 達標，進第二階段。
A5_ucur − A5_acc +3.62（4/5）；A5_ucur − A5 固定頭 pod −1.71（0/5）；B2_ucur − B2_acc **+14.45（5/5）**、
B2_ucur − B2 固定頭 pod +0.66（2/5）。只報告：以當前 C_t 重算 U_old 後，hinge 在累積式頭下恢復
...
### 11.10 DR-055 — Evidence budget sweep：B ∈ {4, 8, 16}（A5 hier、accumulating、`--uold current`、十折兩順序）

新 pod（RTX 5090，`-p 58625`）啟動前先做對數：B = 8、A5 hier、fold 1 重跑與既有紀錄
**569 筆逐位元相同**（class-IL 0.805065 vs 0.805065，差 0.00e+00），故 B = 8 直接引用
§11.9，只加跑 B ∈ {4, 16}（40 run，152 分鐘，零失敗）。唯一變動是 `--budget`。

| B | 順序 | ACC | Masked ACC | Forgetting | BWT |
|---|---|---|---|---|---|
| 4 | reverse | 0.816 ± 0.040 | 0.907 ± 0.022 | 0.110 ± 0.047 | −0.099 ± 0.047 |
| 4 | forward | **0.759 ± 0.091** | 0.856 ± 0.084 | 0.070 ± 0.027 | −0.067 ± 0.028 |
| 8（§11.9） | reverse | 0.834 ± 0.031 | 0.914 ± 0.022 | 0.101 ± 0.035 | −0.094 ± 0.035 |
| 8（§11.9） | forward | 0.837 ± 0.027 | 0.916 ± 0.022 | 0.059 ± 0.020 | −0.046 ± 0.026 |
| 16 | reverse | 0.829 ± 0.036 | 0.917 ± 0.018 | 0.118 ± 0.042 | −0.112 ± 0.046 |
| 16 | forward | 0.847 ± 0.032 | 0.917 ± 0.017 | 0.059 ± 0.031 | −0.049 ± 0.031 |

逐折配對（同折相減）：

| 配對 | 順序 | ACC | Masked ACC | Forgetting |
|---|---|---|---|---|
| B=4 − B=8 | reverse | −0.0180（4/10） | −0.0076（4/10） | +0.0087（5/10） |
| B=4 − B=8 | forward | **−0.0784（1/10）** | −0.0605（3/10） | +0.0106（6/10） |
| B=16 − B=8 | reverse | −0.0047（5/10） | +0.0024（5/10） | +0.0165（B=16 較低 2/10） |
| B=16 − B=8 | forward | +0.0098（6/10） | +0.0005（4/10） | −0.0002（4/10） |

只報告：把預算砍半（B=4）在正向掉 7.8 pp（1/10）且折間變異放大（sd 0.091 vs 0.027），
反向掉 1.8 pp（4/10）；加倍（B=16）兩序都在 ±1 pp 內（5/10、6/10），反向的 Forgetting
反而高 1.7 pp（2/10）。即 B = 8 以下會明顯受限、以上沒有可辨識增益。**不宣稱最佳預算、
