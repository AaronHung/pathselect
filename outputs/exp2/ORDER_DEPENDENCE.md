...
B2（只 eq）的 `l_eq_fire_rate` 為 **0.1114**（n=5 seeds），A5（三項全開）為 **0.0740**（n=5）—— **B2 的 hinge 被觸發的比例約為 A5 的 1.5 倍**。

（本節數字由 `outputs/exp2/ablation/per_slide/*.json` 重算，不寫死；先前寫死的 0.1142 是 3-seed 舊值，與 5-seed 產物對不上 —— DR-044 同批修正。）
