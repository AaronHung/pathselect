...
REPRO_ROOT = ROOT / "outputs" / "exp2" / "sota" / REPRO_SUBDIR
...
def main(argv=None) -> int:
    ...
    # ── 重現檢查（可能有多條線：reverse / forward）──────────────────────────
    def _published(src: str, key: str, idx: int):
        if src == "tab1":
            return TAB1_MAIN[key], TAB1_CITATION
        return {r[0]: r for r in ROWS}[MAIN_METHOD][idx], CITATION

    shown = [(lab, REPRO_ROOT / sub / "summary.json", note, src)
             for lab, sub, note, src in REPRO_RUNS
             if (REPRO_ROOT / sub / "summary.json").is_file()]
    if shown:
        L += ["## 重現檢查（official code, RunPod RTX 4090）", "",
              "跑的是**對方公開的實作**（非我們的重寫），其預設 12 epoch、"
              "同一份 10 折切分。指標由**它自己輸出的準確率矩陣**"
              "（`metrics/test_acc.txt`）套**我們統一的定義**算出"
              "（`sota/repro_metrics.py`），不是它印在畫面上的彙總。"
              "環境與必要改動見 [`docs/repro/`](repro/)。", ""]
    for lab, path, note, src in shown:
        R = json.loads(path.read_text())
        n = R["n"]
        L += [f"### {lab}　（{n} folds）", "", note, "", 
              "| 指標 | 重現 | 發表值 | 差 | 出處 |", "|---|---|---|---|---|"]
        for key, mlab, idx in (("acc", "ACC ↑", 1), ("masked_acc", "Masked ACC ↑", 4),
                               ("forgetting", "Forgetting ↓", 2), ("bwt", "BWT ↑", 3)):
            m, s = R["aggregate"][key]
            pv, cite = _published(src, key, idx)
            d = ""
            if pv:
        ...
        if all(abs(v["forgetting"] + v["bwt"]) < 1e-12 for v in R["per_fold"].values()):
            L += [f"（這 {n} 折全部滿足 `Forgetting == −BWT`：每個任務的準確率峰值都恰好"
                  "落在剛學完它的那一階段、之後不再上升。兩個定義只在「學完之後還會再"
                  "升高」時才分離 —— 本表最上方我們自己的 `A5`／`flat`／reverse 一列即"
                  "為分離的例子。）", ""]
        if R.get("errors"):
    ...
    if shown:
        L += ["### 讀這一節必須連帶的限定", "",
              ...
              "* 兩條線都另套一道 API 相容修補（`np.long` → `np.int64`，語意等價），"
              "見 [`docs/repro/SHIMS.md`](repro/SHIMS.md)。",
              "* **forward 線只改了本機路徑**，順序設定與資料檔一律原樣；"
              "**reverse 線另需重排 `class_ensemble.json` 的鍵序**才跑得起來 ——"
              "其 `get_current_ensemble_classes` 依該檔鍵序決定任務順序、不看 config。",
              "* **裝置一致性對照無法取得**：同折的 CPU 對照在 16 GB 的 Mac 上跑到"
              "第三個任務即被系統終止（OOM）。", "",
              "**因此差距的解讀必須逐線分開**，不能把兩條線的結論混為一談。", ""]
